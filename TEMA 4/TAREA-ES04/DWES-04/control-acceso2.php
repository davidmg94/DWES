...
define("MAX_INTENTOS", 4); // Máximo de intentos fallidos permitidos
define("INTERVALO_PARA_BLOQUEO", 15 * 60); // Tiempo máximo (en segundos) que se permite entre fallos de inicio de sesión antes de bloquear al usuario.
define("DURACION_BLOQUEO", 60 * 60); // Duración del bloqueo en segundos
define("MAX_INTENTOS_SEGUIDOS", 3); // Máximo de intentos fallidos CONSECUTIVOS permitidos
define("INTERVALO_PARA_BLOQUEO_SEGUIDOS", 5 * 60); // Tiempo máximo (en segundos) que se permite entre fallos de inicio de sesión consecutivos antes de bloquear al usuario.
...
define("TIEMPO_BLOQUEO_TOTAL_SEGUIDOS", DURACION_BLOQUEO + INTERVALO_PARA_BLOQUEO_SEGUIDOS);
...
function estaBloqueado($usuario, $conexion) {
    // Verifica si el usuario ha alcanzado el límite de intentos fallidos seguidos
    if (verificarIntentosFallidosSeguidos($usuario, $conexion)) {
        return true;
    }
...
function verificarIntentosFallidosSeguidos($usuario, $conexion) {
    $hora = time(); // Obtiene la hora actual

    // Consulta para obtener los últimos registros de intentos fallidos del usuario
    $stmt = $conexion->prepare("SELECT Hora, Acceso FROM login WHERE Usuario = ? ORDER BY Hora DESC LIMIT " . MAX_INTENTOS_SEGUIDOS);
    $stmt->execute(array($usuario));
    $intentos = $stmt->fetchAll(PDO::FETCH_ASSOC);

    $denegados = 0;

    // Contar el número de intentos denegados entre los últimos registros obtenidos
    ...
    // Si hay por lo menos el número de intentos fallidos permitidos
    // y el tiempo transcurrido desde el último intento fallido (denegado) hasta ahora es menor que el tiempo para bloqueo, el usuario está bloqueado    
    return $denegados >= MAX_INTENTOS_SEGUIDOS && ($hora - $intentos[$denegados - 1]['Hora']) < INTERVALO_PARA_BLOQUEO_SEGUIDOS;
}
